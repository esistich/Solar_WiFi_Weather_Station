<?php
/**
 * SWS Admin – Dashboard, Login & Admin-API
 * Wird von index.php eingebunden (Session/Config bereits geladen).
 */
...
if (($_GET['action'] ?? '') === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	try {
		...
		$row = $stmt->fetch();
		if ($row && password_verify($inputPass, $row['password'])) $loginOk = true;
	} catch (\Throwable $e) {}

	if (!$loginOk && defined('ADMIN_USER') && defined('ADMIN_PASS_HASH')) {
		if ($inputUser === ADMIN_USER && password_verify($inputPass, ADMIN_PASS_HASH)) $loginOk = true;
	}
	if ($loginOk) {
...
// ── Admin JSON-API ──────────────────────────────────────────────────
if ($loggedIn && str_starts_with($_GET['action'] ?? '', 'api/')) {
...
<div class="sws-layout">
  <nav class="sws-sidebar" id="sws-sidebar">
	...
	<ul class="sws-nav">
	  ...
	  <li class="sws-nav-cat">System</li>
	...
	<div class="sws-page-wrap">
	  ...
	  <section id="credentials" class="sws-section" style="display:none">
		...
		<p class="remark">Ändere API-Zugangsdaten, Admin-Passwort oder JWT-Secret.<br>
		  Geänderte API-Zugangsdaten müssen im Station-Sketch eingetragen werden.</p>
		<div class="sws-form-wrap">
		  <div id="cred-msg"></div>
		  <form id="cred-form">
			<div class="form-group">
			  <label>Admin-Passwort <span class="fg-red">*</span></label>
			  <input type="password" name="admin_password" class="metro-input" required autocomplete="current-password" placeholder="Aktuelles Passwort zur Bestätigung">
			</div>
			<details class="sws-collapse">
			  <summary>API-Zugangsdaten (Station → API)</summary>
			  <div class="row">
			...
			<details class="sws-collapse">
			  <summary>JWT-Secret (Flutter-App)</summary>
			  <div class="form-group">
