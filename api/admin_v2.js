...
function toggleSidebar() {
  const sb = document.getElementById('sws-sidebar');
  sb.classList.toggle('collapsed');
  sb.classList.toggle('mobile-open');
  // Overlay nur auf Mobile relevant
...
function showSection(id) {
  ...
  if (window.innerWidth < 768) {
    document.getElementById('sws-sidebar')?.classList.remove('mobile-open');
  }
}
...
// ---- Stationen ----
function stationStatus(r) {
  const sleepMin = (r.settings?.sleep_min) || 10;
  if (!r.last_seen) return '<span class="sws-dot sws-dot-offline" title="Keine Daten">●</span>';
  const ageMin = (Date.now() - tsToLocal(r.last_seen).getTime()) / 60000;
  if (ageMin < sleepMin * 2)       return '<span class="sws-dot sws-dot-ok" title="Online (seit '+Math.round(ageMin)+' min)">●</span>';
  if (ageMin < sleepMin * 6)       return '<span class="sws-dot sws-dot-warn" title="Verzögert (seit '+Math.round(ageMin)+' min)">●</span>';
  return '<span class="sws-dot sws-dot-offline" title="Offline (seit '+Math.round(ageMin)+' min)">●</span>';
}
...
async function loadStations() {
  ...
  el.innerHTML = `<table>
    <thead><tr><th></th><th>ID</th><th>Slug</th><th>Name</th><th>MAC</th><th>Messungen 24h</th><th>Batterie</th><th>WLAN</th><th>Letzte Aktivit&#228;t</th><th>Firmware</th><th>Letzter Fehler</th><th></th><th></th></tr></thead>
    <tbody>${rows.map(r => `<tr>
      <td style="width:24px;text-align:center">${stationStatus(r)}</td>
      <td>${r.id}</td>
      <td><code>${r.slug}</code></td>
      <td>${r.name}${r.temperature ? ' <span class=\\"fg-secondary\\" style=\\"font-size:.82rem\\">'+Number(r.temperature).toFixed(1)+'°C</span>' : ''}</td>
      <td class="fg-secondary" style="font-size:.82rem;font-family:monospace">${r.mac ?? '\u2013'}</td>
      <td style="font-size:.82rem">${r.meas_24h ?? 0}</td>
      <td>${batteryBadge(r.battery_pct)}</td>
      <td>${wifiBadge(r.wifi_strength)}</td>
      ...
      <td>${lastErrorBadge(r)}</td>
      <td><button class="btn-edit" onclick="editStation(${r.id},'${esc(r.slug)}','${esc(r.name)}','${r.mac??''}','${encodeURIComponent(JSON.stringify(r.settings??{}))}')">Bearbeiten</button></td>
  ...
  if (summary) {
    let online = 0, warn = 0, offline = 0, lowBat = 0;
    rows.forEach(r => {
      const s = stationStatus(r);
      if (s.includes('sws-dot-ok')) online++;
      else if (s.includes('sws-dot-warn')) warn++;
      else offline++;
...
function batteryBadge(pct) {
  if (pct === null || pct === undefined || pct === '') return '<span class="fg-secondary">\u2013</span>';
  const v = Number(pct);
  const cls = v < 20 ? 'badge-error' : v < 40 ? 'badge-warning' : 'badge-info';
  return `<span class="badge ${cls}">${v}%</span>`;
}

function wifiBadge(dBm) {
  if (dBm === null || dBm === undefined || dBm === '') return '<span class="fg-secondary">\u2013</span>';
  const v = Number(dBm);
  const cls = v < -85 ? 'badge-error' : v < -75 ? 'badge-warning' : 'badge-info';
  return `<span class="badge ${cls}">${v} dBm</span>`;
}

function lastErrorBadge(r) {
...
function editStation(id, slug, name, mac = '', settingsJson = '{}') {
  const s = JSON.parse(decodeURIComponent(settingsJson));
  openEditModal('Station bearbeiten', [
    ...
    { label: 'Schlafintervall (min)', name: 'sleep_min', value: s.sleep_min ?? 10, type: 'number', hint: 'Minuten zwischen Messungen' },
    { label: 'Temp.-Korrektur (°C)',  name: 'temp_corr', value: s.temp_corr ?? 0, type: 'number', hint: 'z.B. -1.5 für zu warm messenden Sensor', step: '0.1' },
    { label: 'Höhe (m)',             name: 'elevation', value: s.elevation ?? 420, type: 'number', hint: 'Für relative Druckberechnung' },
    { label: 'API-Pfad',             name: 'api_path', value: s.api_path ?? '/sws/api/v1/data', type: 'text', hint: 'Nur ändern wenn API umzieht' },
  ], async data => {
    const body = { id, name: data.name, slug: data.slug };
    if (data.mac !== undefined) body.mac = data.mac || null;
    body.settings = {
      sleep_min: parseInt(data.sleep_min) || 10,
      temp_corr: parseFloat(data.temp_corr) || 0,
      elevation: parseInt(data.elevation) || 420,
      api_path:  data.api_path || '/sws/api/v1/data'
    };
...
function startStationsRefresh() {
  clearInterval(stationsTimer);
  stationsTimer = setInterval(loadStations, 60_000); // jede Minute
}

function updateRefreshTime() {
  const el = document.getElementById('refresh-time');
  if (el) el.textContent = new Date().toLocaleTimeString('de-DE', { timeZone: 'Europe/Berlin' });
}

// ---- Server-Info ----
async function loadServerInfo() {
  const info = await api('serverinfo');
  const el = document.getElementById('serverinfo-content');
  if (!info) { el.innerHTML = '<p class="fg-secondary">Fehler beim Laden.</p>'; return; }

  const extIcon = ok => ok ? '<span class="fg-green mif-checkmark"></span>' : '<span class="fg-red mif-cross"></span>';

  el.innerHTML = `
    <div class="sws-info-grid">
      <div class="sws-info-card">
        <h4><span class="mif-cog"></span> Server</h4>
        <table class="sws-info-table">
          <tr><td>PHP-Version</td><td><code>${info.php_version}</code></td></tr>
          <tr><td>Memory Limit</td><td><code>${info.php_memory}</code></td></tr>
          <tr><td>Max Execution</td><td><code>${info.php_max_exec}</code></td></tr>
          <tr><td>POST Max Size</td><td><code>${info.php_post_max}</code></td></tr>
          <tr><td>Upload Max</td><td><code>${info.php_upload_max}</code></td></tr>
          ${info.disk_free_mb ? `<tr><td>Speicher frei</td><td><code>${info.disk_free_mb} MB</code> / ${info.disk_total_mb} MB</td></tr>` : ''}
        </table>
      </div>
      <div class="sws-info-card">
        <h4><span class="mif-database"></span> Datenbank</h4>
        <table class="sws-info-table">
          <tr><td>Server</td><td><code>${info.db_version}</code></td></tr>
          <tr><td>Datenbank</td><td><code>${info.db_name}</code></td></tr>
          ${Object.entries(info.tables).map(([k,v]) => `<tr><td>${k}</td><td><code>${v !== null ? v.toLocaleString('de-DE') : '?'}</code> Zeilen</td></tr>`).join('')}
        </table>
      </div>
      <div class="sws-info-card">
        <h4><span class="mif-extension"></span> Erweiterungen</h4>
        <table class="sws-info-table">
          ${Object.entries(info.extensions).map(([k,v]) => `<tr><td>${k}</td><td>${extIcon(v)}</td></tr>`).join('')}
        </table>
      </div>
    </div>`;
}
...
async function loadLogStats() {
  const days = document.getElementById('logstats-days')?.value ?? '7';
  const data = await api(`logstats&days=${days}`);
  const el = document.getElementById('logstats-content');
  if (!data) { el.innerHTML = '<p class="fg-secondary">Fehler beim Laden.</p>'; return; }
  ...
  if (data.station_errors.by_station.length) {
    html += '<div style="margin-top:8px;font-size:.82rem">' + data.station_errors.by_station.map(s => `<div>${s.slug}: ${s.cnt}</div>`).join('') + '</div>';
  }
  html += '</div>';
  ...
  if (data.system_log.by_level.length) {
    html += '<div style="margin-top:4px">' + data.system_log.by_level.map(l => `<span class="badge ${badgeClass(l.level)}">${l.level}: ${l.cnt}</span> `).join('') + '</div>';
  }
  html += '</div>';
...
async function loadSystemLog() {
  ...
  const res  = await fetch(`?${params}`, { credentials: 'same-origin' });
  const data = await res.json();
  const rows = Array.isArray(data) ? data : (data.rows ?? []);

  const el = document.getElementById('systemlog-table');
  if (!rows.length) { el.innerHTML = '<p class="fg-secondary">Keine Einträge.</p>'; return; }

  const badgeClass = lvl => ({ error:'badge-error', warning:'badge-warning', info:'badge-info' }[lvl] ?? '');
  const sourceBadge = src => {
    const map = { auth:'badge-auth', station:'badge-station', api:'badge-api', db:'badge-db', router:'badge-router' };
    return map[src] ?? '';
  };

  el.innerHTML = `<table><thead><tr>
    <th>Zeit</th><th>Level</th><th>Quelle</th><th>Code</th><th>Nachricht</th><th>IP</th><th>Kontext</th>
  </tr></thead><tbody>` +
  rows.map(r => `<tr>
    <td class="fg-secondary" style="white-space:nowrap;font-size:.8rem">${fmtTs(r.created_at)}</td>
    <td><span class="badge ${badgeClass(r.level)}">${r.level}</span></td>
    <td><span class="badge ${sourceBadge(r.source)}">${r.source}</span></td>
    <td style="font-family:monospace;font-size:.82rem">${r.code}</td>
    <td>${r.message}</td>
    <td class="fg-secondary" style="font-family:monospace;font-size:.78rem">${r.ip ?? ''}</td>
