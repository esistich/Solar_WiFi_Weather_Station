...
--
-- REIHENFOLGE:
--   1. Dieses Skript in der Datenbank ausf�hren
--   2. Anschlie�end neue API v1 deployen
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `stations` (
  ...
  `api_key`    VARCHAR(64)   NULL     COMMENT 'Zuk�nftig: stationsspezifischer API-Key',
...
-- Standardstation anlegen (f�r bestehende Daten)
INSERT IGNORE INTO `stations` (`id`, `slug`, `name`) VALUES (1, 'sws-main', 'SWS Hauptstation');

-- ----------------------------------------------------------
-- 2. measurements � station_id + device_ts hinzuf�gen
-- ----------------------------------------------------------
...
INSERT IGNORE INTO `metric_definitions` (`metric_key`, `label`, `unit`, `display_order`, `chart_color`) VALUES
  ('temperature',     'Temperatur Au�en',  '�C',  1,  '#e15759'),
...
--    (nur Felder die numerisch sind; Strings werden �bersprungen)
-- ----------------------------------------------------------
...
-- Alte Spalten in measurements k�nnen nach Verifikation
-- mit ALTER TABLE measurements DROP COLUMN ... entfernt werden.
-- ============================================================

-- ----------------------------------------------------------
-- Bestehende users-Tabelle: role-Spalte nachr�sten (idempotent)
-- ----------------------------------------------------------
ALTER TABLE `users`
  ADD COLUMN IF NOT EXISTS `role` ENUM('user','admin') NOT NULL DEFAULT 'user' AFTER `password`;

-- ----------------------------------------------------------
-- Nachtr�gliche Korrekturen (idempotent ausf�hrbar)
-- ----------------------------------------------------------
-- value-Spalte auf VARCHAR erweitern damit String-Metriken
-- (pressure_state, zambretti, trend) gespeichert werden k�nnen.
...
-- Bestehende Installationen: role-Spalte nachtr�glich hinzuf�gen.
-- Bereits vorhandene Admins m�ssen danach im Admin-Dashboard
